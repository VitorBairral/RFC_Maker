...
cav = Maker.Cavity()
settings = {
        "name" : "(20, 6)", #Sets the name of the output file
        "thickness" : 0.1,
        ...
        "cell_length": 30.0,
        "cell_radius": 22.96,
        "bp_length": 20.0, #Beampipe length (the beampipe is required to simulate the side walls of cavity)
        "bp_radius": 6.0, #Beampipe radius.
...
def tester(x_max, y_max, step, step_raio):
    ...
    while start_y <= y_max:
        while start_x <= x_max:
            ...
            if f"{settings["name"]}.SFO" not in os.listdir(r"c:\Users\vitor25023\Documents\Teste em lote\Teste E0T"):
                cav.fit(settings)
                cav.generate()
                nome_af = settings["name"] + ".af"
                os.startfile(nome_af)
                time.sleep(5 + (start_x * start_y)/10)
            start_x += step
...
def deletar():
    diretorio = r"c:\Users\vitor25023\Documents\Teste em lote\Teste E0T"
